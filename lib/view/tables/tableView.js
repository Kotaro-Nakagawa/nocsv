 ...
 * @param {Number[]} columnsWeight 
 * @returns 
 */
const columnInfoSet = (columns, x, width, maxDepth, columnsWeight) => {
  const columnTotalWeight = columnsWeight.reduce((a, c) => a + c);
  ...
  const columnWidths = columnsWeight.map(w => weightToWidth(w))
...
class TableView extends NodeElement {
  ...
  constructor(table, x, y, width) {
    ...
    const columnsInfo = columnInfoSet(table.getColumns(), x, width, Math.max(...table._rows.map(r => r.depth)), table.styles.getColumnsWeightList(table.getColumns()))
    let currentY = y;
