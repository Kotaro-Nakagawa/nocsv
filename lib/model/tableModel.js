...
class TableModel {
  ...
  constructor(title, json) {
    ...
    this.#styles = new StyleModel(json[FormatKeywords.STYLES])
    this.#rules = new RuleModel(json[FormatKeywords.RULES], json[FormatKeywords.COLUMNS])
  }
  ...
  /**
   * 
   * @typedef RowDepthAndFolding
   * @property {Boolean | undefined} isFolding
   * @property {Number} depth
   * @return {RowDepthAndFolding[]} 
   */
  getRowDepthsAndFoldings() {
    return this._rows.map(row => { return { depth: row.depth, isFolding: row.isFolding } })
  }
  getColumns() {
  ...
  toJson() {
    ...
    return [this.titleRow.title, {
      ...
      [FormatKeywords.RULES]: this.#rules.toJson(columns),
      [FormatKeywords.STYLES]: this.#styles.toJson(columns)
    }]
  }
}
