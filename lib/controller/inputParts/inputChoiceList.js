...
class InputChoiceList {
  ...
  #baseHeight() {
    return parseInt(this.#baseElement.style.height, 10)
  }

  moveTo(top, left) {
    this.#textBoxLeft = parseInt(left, 10)
    this.#textBoxTop = parseInt(top, 10)
    this.#baseElement.style.top = `${this.#textBoxTop - this.#baseHeight()}px`
   ...
   */
  #getFilteredList() {
    return this.#validation.examples.filter(e => e.startsWith(this.#filterValue))
  }
  /**
  ...
  #reloadElements() {
    ...
    if (list.length === 0) {
      this.#validate(this.#filterValue)
    }
