...
class ControllerManager {
   ...
   */
  #onWheel(e) {
    return this.#scrollView(e.deltaY)
  }

  #scrollView(direction) {
    const scrollResult = direction > 0 ? this.#display.scrollDown() : this.#display.scrollUp()
    this.#reflectChange()
    this.#adjustInputWindowPosition()
    return scrollResult
  }
  ...
  #keepCursorInWindow() {
    ...
    if (cursorArea.bottom > h) {
      this.#display.scrollDown(cursorArea.bottom - h)
    }
    if (cursorArea.top < 0) {
      this.#display.scrollUp(Math.abs(cursorArea.top))
    }
  }
  ...
  #onPageDown() {
    ...
    this.#display.scrollDown(this.#view.getAreaOfPath(this.#currentPosition.getPath()).top)
    this.#reflectChange()
    this.#adjustInputWindowPosition()
  }
